...
async function saveData(year: string, semester: string, data: ScrapedDataRow[]) {
  ...
  try {
    ...
    const extractionId = Number(extractionResult.lastInsertRowid);
...
export async function scrapeUFCData(formData: FormData): Promise<{ success: boolean; data?: ScrapedDataRow[]; error?: string, logs?: string[], cancelled?: boolean }> {
    ...
    if(visibleMode) {
        addLog("Modo visível ATIVADO. A janela do navegador será exibida.");
    } else {
        addLog("Modo visível DESATIVADO. O processo será executado em segundo plano.");
    }
    ...
    try {
        ...
        // Limita a 3 turmas para teste
        turmasParaProcessar = turmasParaProcessar.slice(0, 3);
        addLog(`[MODO DE TESTE] Limitando a extração para ${turmasParaProcessar.length} turmas.`);
        ...
        for (let i = 0; i < turmasParaProcessar.length; i++) {
            ...
            try {
                ...
                if (e.name === 'AbortError' || e.message.includes('cancelado')) throw e;
        ...
        if (scrapedData.length > 0) {
            addLog("Etapa 8: Salvando dados brutos no banco de dados...");
            const saveResult = await saveData(year, semester, scrapedData);
            if (!saveResult.success || !saveResult.extractionId) {
                throw new Error(saveResult.error || "Falha ao obter ID da extração.");
            }
            addLog("Dados brutos salvos com sucesso.");
            
            addLog("Etapa 9: Processando dados e gerando arquivos CSV...");
            const processResult = await processData(scrapedData, `${year}.${semester}`);
            
            addLog("Etapa 10: Salvando arquivos processados no banco de dados...");
            const saveFilesResult = await saveProcessedFiles(saveResult.extractionId, processResult);
             if (!saveFilesResult.success) {
                throw new Error(saveFilesResult.error);
            }
            addLog("Arquivos processados salvos com sucesso.");

        } else {
            addLog("Nenhum dado foi extraído. Processo encerrado.");
        }

        addLog("Extração e processamento concluídos com sucesso.");
        return { success: true, data: scrapedData, logs };

    } catch (e: any) {
        if (e.message.includes('cancelado')) {
            addError('Processo cancelado pelo usuário.');
            return { success: false, error: 'Extração cancelada.', logs, cancelled: true };
        }
